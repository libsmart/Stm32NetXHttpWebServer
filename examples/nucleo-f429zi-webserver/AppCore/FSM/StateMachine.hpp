...
#ifndef NUCLEO_F429ZI_WEBSERVER_APPCORE_FSM_STATEMACHINE_HPP
#define NUCLEO_F429ZI_WEBSERVER_APPCORE_FSM_STATEMACHINE_HPP
...
namespace AppCore::FSM {
    ...
    public:
        ...
        //     void onTransition(Args...) {}
        //
        //     template<typename FromState, typename ToState>
        //     void onTransition(FromState &fromState, ToState &toState) {}
    };
    ...
    public:
        ...
        auto handle(const Event &event) -> Status {
            auto passEventToState = [this, &event](auto statePtr) noexcept {
                ...
                try {
                    ...
                    return FSM::Status{FSM::TerminateWithError{err.what()}};
                } catch (...) {
                    return FSM::Status{FSM::TerminateWithError{"not std::exception"}};
                }
#else
                auto action = statePtr->handle(event);
                return action.Execute(*this, *statePtr, event);
        ...
        auto transitionTo() -> State & {
            auto &toState = std::get<State>(states_);

            /*
            // auto &toState = getState<State>();
            auto fromState = currentState();
            // Transition(fromState, toState);

            Stm32ItmLogger::logger.printf("%s: ", __FUNCTION__);
            auto toSt = dynamic_cast<Nameable *>(&toState);

            std::visit([toSt](auto &&fromState) {
                auto fromSt = dynamic_cast<Nameable *>(fromState);
                if (fromSt && toSt) {
                    Stm32ItmLogger::logger.printf("%s => %s\r\n", fromSt->getName(), toSt->getName());
                }
            }, current_state_);
            */
         ...
         */
        template<typename State>
        [[nodiscard]] auto isIn() const -> bool {
    ...
    private:
        // template<typename... Args>
        // void Transition(Args...) {
        // Stm32ItmLogger::logger.println("NOOOOOOOOOOOOO");
        // }

        template<typename FromState, typename ToState>
        [[maybe_unused]] auto Transition(FromState &fromState, ToState &toState)
            -> decltype(onTransition(fromState, toState)) {
            return onTransition(fromState, toState);
            // return std::visit([&machine, &fromState, &toState]() noexcept {
            // return machine.onTransition(&fromState, &toState);
            // });
        };
    ...
    private:
        ...
        template<typename... Args>
        static auto Leave(Args... /*unused*/) -> Status {
            return Continue{};
        }
        ...
        // lowest ranking for overload resolution.
        template<typename... Args>
        static auto Enter(Args... /*unused*/) -> Status {
            return Continue{};
        }
    ...
    struct OneOf {
        ...
        auto Execute(Machine &machine, State &state, const Event &event) -> Status {
            return std::visit(
                [&machine, &state, &event](
            auto &action) {
                    return action.Execute(machine, state, event);
                },
                option);
        }
